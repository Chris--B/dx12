...
impl Renderer {
    ...
    pub fn create(config: &config::Config,
        ...
        let backbuffer_format = DXGI_FORMAT_R11G11B10_FLOAT;

        let mut msaa_quality_lvls = D3D12_FEATURE_DATA_MULTISAMPLE_QUALITY_LEVELS {
            Format:             backbuffer_format,
            SampleCount:        4,
            Flags:              D3D12_MULTISAMPLE_QUALITY_LEVELS_FLAG_NONE,
            NumQualityLevels:   0,
        };
        check_feature_multisample_quality(&device, &mut msaa_quality_lvls)?;
        let msaa_4x_quality = msaa_quality_lvls.NumQualityLevels;
        println!("4x MSAA Quality: {}",  msaa_4x_quality);
        ...
        println!("{:#?}\n", swapchain_desc);
        let swapchain = create_swapchain(&dxgi_factory,
                                         &cmd_queue,
                                         h_wnd,
                                         swapchain_desc)?;
